/**********************************************************************\
 @file main_make_mock_lenses.cpp
 ------------------
...
const std::string output_root = "_mock_lens.dat";

constexpr unsigned num_lenses_to_generate = 100000;
...
int main( const int argc, const char *argv[] )
{
	...
	for(unsigned i=0;i<num_fields;++i)
	{
		...
		std::string pixel_map_name = field_directory + field_name_root + lens_pixel_map_root;
		std::string field_output_file_name = field_directory + field_name_root + output_root;
		...
		for( size_t j=0; j<num_lenses_to_generate; ++j )
		{
			...
			field_output_table["dec_radians"].push_back(galaxy_sky_positions[j].second.first*brgastro::unitconv::degtorad);
			...
			field_output_table["EXTINCTION_"].push_back(0);
		}
...
#endif
		{
			brgastro::print_table_map(field_output_file_name,field_output_table);
		}
	}
